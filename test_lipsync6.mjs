...
// audio_type: "url" seems most logical
const body = {
  input: {
    video_url: VIDEO_URL,
    audio_url: AUDIO_URL,
...
console.log("=== Kling lip-sync with audio_type=url ===");
...
console.log("Response:", t.slice(0, 600));

if (r.ok) {
  const j = JSON.parse(t);
  if (j.data?.task_id) {
    console.log("\n>>> Polling task:", j.data.task_id);
    for (let i = 0; i < 80; i++) {
      ...
      if (st === "succeed") {
        console.log("SUCCESS:", JSON.stringify(pj.data).slice(0, 2000));
        break;
      }
      if (st === "failed") {
        console.log("FAILED:", pj.data?.task_status_msg);
        break;
      }
    }
  }
